...
const COLUMN_MAPPINGS = {
  designation: ['désignation', 'designation', 'produit', 'libellé', 'libelle', 'article', 'description'],
...
async function parseExcelFile(file: File): Promise<ParsingResult> {
  try {
    ...
    for (let i = headerRowIndex + 1; i < data.length; i++) {
      ...
      const quantite = colIndexes.quantite !== -1 
        ? extractNumber(row[colIndexes.quantite]) 
        : 1;
      
      const prix_unitaire_ht = colIndexes.prix_unitaire !== -1
        ? extractNumber(row[colIndexes.prix_unitaire])
        : 0;
      
      const total_ligne_ht = extractNumber(row[colIndexes.total_ligne]);
      
      const remise_pourcent = colIndexes.remise_pourcent !== -1
        ? extractNumber(row[colIndexes.remise_pourcent])
        : undefined;
      
      const remise_montant = colIndexes.remise_montant !== -1
    ...
    for (let i = 0; i < Math.min(10, headerRowIndex); i++) {
      ...
      if (firstCell.includes('facture') || firstCell.includes('n°') || firstCell.includes('numero')) {
...
export function validateParsedInvoice(data: FactureParsee): { valid: boolean; errors: string[] } {
  ...
  const calculatedNet = data.total_brut_ht - data.total_remises_lignes - (data.remises_pied_facture || 0);
  const ecartPourcent = Math.abs((calculatedNet - data.net_a_payer) / data.net_a_payer * 100);
  
  if (ecartPourcent > 5) {
    errors.push(`Incohérence détectée : Net calculé (${calculatedNet.toFixed(2)}€) vs Net facture (${data.net_a_payer.toFixed(2)}€)`);
  }
